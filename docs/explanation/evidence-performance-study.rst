...
For entry-point guidance, start with :doc:`performance`. This page preserves
the measurements and optimization investigation behind that guidance.

Deciding conformance answers one bit per ``(statement, focus)`` pair and may
stop at the first thing that settles it. Materializing evidence builds and
retains a derivation. The measurements below compare those paths and record
the effects of several optimizations.
