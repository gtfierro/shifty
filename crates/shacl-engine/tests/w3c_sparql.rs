...
//!   * the algebra evaluator (`shacl_engine::validate`), and
//!   * the RDF-driven W3C report validator (`shacl_engine::validate_report`).
//!
...
fn w3c_sparql_conformance() {
    ...
    for file in &files {
        ...
        let parsed = match shacl_parse::parse_turtle(&bytes, None) {
        ...
        match shacl_engine::validate(&loaded.graph, &parsed.schema) {
