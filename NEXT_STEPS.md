...
- Interpreter → ShapeIR: start iterating `context.shape_ir().node_shapes` / `property_shapes` in validation instead of `model.node_shapes`/`prop_shapes`; use IDs to fetch runtime components from the existing map. Keep behaviour identical while the IR proves out.
