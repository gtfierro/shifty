...
//!
//! Run with: `cargo run --release -p shacl-engine --example bench`
//!
...
fn main() {
    ...
    let parsed = shacl_parse::parse_turtle(ttl.as_bytes(), None).unwrap();
    ...
    let t = Instant::now();
    for _ in 0..runs {
        reference_conforms = shacl_engine::validate(&loaded.graph, &parsed.schema).unwrap().conforms;
    }
    ...
    let t = Instant::now();
    for _ in 0..runs {
        planned_conforms = shacl_engine::validate_plan(&loaded.graph, &physical).unwrap().conforms;
    }
