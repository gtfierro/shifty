...
| **AF-CC** | **Custom constraint components** (`sh:parameter` + `sh:validator`/`sh:nodeValidator`/`sh:propertyValidator`) | 🚪 *(done, report path)* | `Reporter::collect_components`: activate a component when the shape supplies every mandatory parameter, pre-bind the parameter values (var = path local name) + `$this`/`$value`/`$PATH`/`$currentShape`, run the validator (ASK per value node, false ⇒ violation; SELECT per focus, each row ⇒ violation). Algebra path diagnoses activation. `$PATH` handles simple + complex paths (shared `apply_path_prebinding`); JS validators unsupported. |
| **AF-F** | **SHACL functions** (`sh:SPARQLFunction`, `sh:JSFunction`) | 🚪 *(done, SPARQL)* | evaluated in node expressions (full graph access) and registered as custom SPARQL functions (`evaluator()` via Oxigraph `with_custom_function`) so calls resolve inside any query; `evaluate_function_expression` for `dash:expression`. Pure-of-arguments only (empty-dataset body); graph-reading bodies in SPARQL contexts and JS functions unsupported. |
