...
fn is_inferencing(loaded: &shacl_parse::Loaded) -> bool {
    loaded
        .graph
...
fn expected_inferred(loaded: &shacl_parse::Loaded) -> Vec<Triple> {
    let mut out = Vec::new();
    for case in loaded
        .graph
        .subjects_for_predicate_object(vocab::RDF_TYPE, DASH_INFERENCING_TEST_CASE)
        ...
        .collect::<Vec<_>>()
    {
        for er in loaded.objects(&case, DASH_EXPECTED_RESULT) {
            let Some(ern) = shacl_parse::graph::term_to_node(&er) else { continue };
            ...
            let subj: NamedOrBlankNode = match shacl_parse::graph::term_to_node(&s) {
                Some(n) => n,
                None => continue,
            };
...
fn expected_report(loaded: &shacl_parse::Loaded) -> Option<(bool, Vec<Key>)> {
...
fn w3c_advanced_conformance() {
    ...
    for file in &files {
        ...
        let Ok(loaded) = shacl_parse::load_turtle(&bytes, Some(&base)) else {
        ...
        if is_inferencing(&loaded) {
            ...
            let parsed = match shacl_parse::parse_turtle(&bytes, Some(&base)) {
            ...
            match shacl_engine::infer(&loaded.graph, &parsed.schema) {
