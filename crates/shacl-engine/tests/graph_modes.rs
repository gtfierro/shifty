...
fn implicit_class_targets_follow_the_shapes_class_hierarchy() {
    ...
    "#;
    let loaded = shacl_parse::load_turtle(ttl, None).unwrap();
    let parsed = shacl_parse::parse_loaded(&loaded);

    let outcome = shacl_engine::validate(&loaded.graph, &parsed.schema).unwrap();
